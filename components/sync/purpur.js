...
async function syncPurpur(path) {
    ...
    try {
            ...
            try {
                ...
                try {
                    ...
                    if (config.mirrorMode === true) {
                        ...
                        jsonData["purpur"][version] = {
                            "file": `https://api.purpurmc.org/v2/purpur/${version}/${latestBuilds}/download`,
                            "mode": "full"
                        };
                    }
        ...
        if (config.mirrorMode === false) {
            fs.writeFileSync(path + `/temp/purpur.json`, JSON.stringify(jsonData, null, 4));
            logger.info(`purpur端Json输出完成!`)
        }
    } catch (error) {
